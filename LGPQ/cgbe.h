...
public:
	...
	void decryption_NL(mpz_t& c, mpz_t& m);
	...
	void setCombinedPrivateKey_NL(int _n);
...
CGBE::CGBE() {
	...
	mpz_init(combined_private_key7);
...
CGBE::~CGBE() {
	...
	mpz_clear(combined_private_key7);
}
...
void CGBE::setCombinedPrivateKey_NL(int cnt) {
	mpz_t _cnt;
	mpz_init(_cnt);
	mpz_set_d(_cnt, cnt);
	mpz_powm(combined_private_key7, gx_1, _cnt, n);
	mpz_clear(_cnt);
}
...
void CGBE::decryption_NL(mpz_t& c, mpz_t& m) {
	// m = c * gx_1 % n
	_mulmod(m, c, combined_private_key7, n);
